...
export const dateFormatter = (site, interval, longForm) => {
...
export const GraphTooltip = (site, graphData, metric) => {
	return (context) => {
    ...
    const offset = document.getElementById("main-graph-canvas").getBoundingClientRect()
      ...
      tooltipEl.style.opacity = 1;
		...
		function renderLabel(label, prev_label) {
			const formattedLabel = dateFormatter(site, graphData.interval, true)(label)
			const prev_formattedLabel = prev_label && dateFormatter(site, graphData.interval, true)(prev_label)
			...
			if (graphData.interval === 'hour') {
				return !prev_label ? `${dateFormatter(site, "date", true)(label)}, ${formattedLabel}` : `${dateFormatter(site, "date", true)(prev_label)}, ${dateFormatter(site, graphData.interval, true)(prev_label)}`
			}
